...
# consecuentemente al banco de preguntas para acceder a la plantilla
  ...
  def __init__(self,curso, grupo):
    ...
    answ=open('Respuestas.txt', 'a') #<- Escritura del archivo de respuestas
    answ.write(' ' + self.id_pregunta + ". " + self.enunciado + '\n' + self.respuestaC +'\n')
    ...
    w=int(input('¿Cuántas opciones equivocadas tendrán las preguntas generadas con esta plantilla?, si es libre digite 0'))
    ...
    if w>0:
      ...
      self.preg=self.enunciado
      blanco="________________________________________________________________________________________"
      self.preg=self.preg+'\n'+ blanco            
#def setCurso(self, c):
#  self.curso=c